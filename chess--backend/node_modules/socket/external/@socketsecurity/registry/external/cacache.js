...
const require$$0$2 = require('node:crypto')
const require$$0$5 = require('node:fs/promises')
const require$$0$3 = require('node:path')
const require$$0$4 = require('node:util')
const require$$1$2 = require('node:os')
const require$$4 = require('node:url')
const require$$2$2 = require('node:fs')
const require$$2$3 = require('node:url')
const require$$1$3 = require('node:path')
const require$$4$1 = require('node:fs')
...
function requireMoveFile() {
  ...
  const { dirname, isAbsolute, join, relative, resolve } = require$$0$3
  const fs = require$$0$5
  ...
  const moveFile = async (
    ...
    if (root) {
      await Promise.all(
        symlinks.map(
          async ({ destination: symDestination, source: symSource }) => {
...
function requireRead() {
  ...
  async function read$1(cache, integrity, opts = {}) {
    const { size } = opts
    const { cpath, sri, stat } = await withContentSri(
      cache,
      integrity,
      async (cpath, sri) => {
        // get size
...
function requireWrite() {
  ...
  async function write$1(cache, data, opts = {}) {
    const { algorithms, integrity, size } = opts
...
async function pMap(
  iterable,
  mapper,
  { concurrency = Number.POSITIVE_INFINITY, signal, stopOnError = true } = {}
) {
