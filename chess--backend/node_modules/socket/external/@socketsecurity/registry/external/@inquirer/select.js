 ...
 */
function infinite({ active, lastActive, pageSize, pointer, total }) {
  if (total <= pageSize) {
    return active
  }
...
function usePagination({ active, items, loop = true, pageSize, renderItem }) {
  const state = useRef({
    position: 0,
    lastActive: 0
  })
...
function createPrompt(view) {
  ...
  const prompt = (config, context = {}) => {
    ...
    const { promise, reject, resolve } = PromisePolyfill.withResolver()
...
const index = createPrompt((config, done) => {
  ...
  const page = usePagination({
    items,
    active,
    renderItem({ index, isActive, item }) {
