...
const require$$0$3 = require('node:url')
...
const require$$2$1 = require('node:string_decoder')
const require$$1$4 = require('node:path')
const require$$0$4 = require('node:child_process')
const require$$1$5 = require('node:os')
const require$$1$3 = require('node:fs/promises')
const require$$1$6 = require('node:os')
const require$$1$7 = require('node:path/win32')
const require$$0$5 = require('node:module')
const require$$0$6 = require('node:process')
  ...
  ;(function (exports) {
    ...
    class Minimatch {
      ...
      matchOne(file, pattern, partial = false) {
        ...
        for (
          let fi = 0, pi = 0, fl = file.length, pl = pattern.length;
          fi < fl && pi < pl;
          fi++, pi++
        ) {
...
function requireCommonjs$1() {
  ...
  class PathScurryBase {
    ...
    constructor(
      ...
      pathImpl,
      sep,
      { childrenCacheSize = 16 * 1024, fs = defaultFS, nocase } = {}
    ) {
    ...
    async walk(entry = this.cwd, opts = {}) {
      ...
      const { filter, follow = false, walkFilter, withFileTypes = true } = opts
      const results = []
...
function requireLib$7() {
  ...
  const { delimiter, join, posix, sep } = require$$1$4
...
function requireLib$6() {
  ...
  const stdioResult = (stdout, stderr, { stdio, stdioString = true }) => {
    const result = {
      stdout: null,
      stderr: null
    }
...
function requireOpts() {
  ...
  const { askPassSetInConfig, sshCommandSetInConfig } = checkGitConfigs()
...
function requireLib$3() {
  ...
  const { basename, join } = require$$1$4
  const normalize = pkg =>
    !pkg.bin
      ? removeBin(pkg)
...
function requireNormalize() {
  ...
  const normalize = async (
    pkg,
    { allowLegacyCase, changes, root, steps, strict }
  ) => {
    if (!pkg.content) {
...
function requireLib() {
  ...
  const { parse, read } = requireReadPackage()
