...
function requireJoinPath() {
  ...
  function build(root, options) {
    const { includeBasePath, relativePaths } = options
...
function requirePushDirectory() {
  ...
  const empty = () => {}
  function build(root, options) {
    const { filters, includeDirs, relativePaths } = options
    if (!includeDirs) {
      return empty
    }
    if (relativePaths) {
...
function requireInvokeCallback() {
  ...
  function build(options, isSynchronous) {
    const { group, maxFiles, onlyCounts } = options
    if (onlyCounts) {
    ...
    walk = (entries, directoryPath, depth) => {
      ...
      for (let i = 0; i < entries.length; ++i) {
        ...
        if (
          ...
          const path = joinPath.joinDirectoryPath(
            entry.name,
            directoryPath,
          ...
          const path = joinPath.joinPathWithBasePath(entry.name, directoryPath)
...
function requireAsync() {
  ...
  function callback(root, options, callback) {
    const walker = new walker_1.Walker(root, options, callback)
    walker.start()
  }
...
function requireDist() {
  ...
  function processPatterns(
    { expandDirectories = true, ignore = [], patterns },
    cwd,
    props
  ) {
